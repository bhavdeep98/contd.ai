...
class IdempotencyGuard:
    ...
    def allocate_attempt(
        ...
        for attempt_id in range(1, self.MAX_ATTEMPTS + 1):
            try:
                ...
                """, workflow_id, org_id, step_id, attempt_id, utcnow(), lease.token)
                
                if result:
                    logger.debug(f"Allocated attempt: {workflow_id}/{step_id}#{attempt_id}")
                    return attempt_id
        ...
        raise TooManyAttempts(f"Step {workflow_id}/{step_id} exceeded {self.MAX_ATTEMPTS} attempts")
    
    def _validate_fencing_token(self, workflow_id: str, org_id: str, token: int) -> bool:
    ...
    def mark_completed(
        ...
        """, workflow_id, org_id, step_id, attempt_id, utcnow(), snapshot_ref, checksum)
