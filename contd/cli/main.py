...
def get_engine(config: ContdConfig):
    ...
    adapter = create_adapter(config.storage_backend, {
...
@click.option('--input-file', '-f', type=click.Path(exists=True), help='JSON input file')
...
def run(ctx, workflow_name, input_data, input_file, workflow_id, run_async):
    ...
    if not workflow_fn:
        ...
        if available:
            ...
            click.echo("No workflows registered. Add workflow modules to contd.json", err=True)
        sys.exit(1)
...
def status(ctx, workflow_id):
    ...
    try:
        ...
        if status_info.get('has_lease'):
            ...
            click.echo(f"  Status: SUSPENDED")
            click.echo(f"  Last step: {status_info.get('latest_snapshot_step', 'unknown')}")
        else:
...
def resume(ctx, workflow_id):
    ...
    try:
        ...
        click.echo(f"  Restoring from step: {status_info.get('latest_snapshot_step', 0)}")
        ...
        click.echo(f"Workflow resumed successfully")
...
def inspect(ctx, workflow_id, verbose):
    ...
    try:
        ...
        try:
            ...
            if verbose:
                click.echo(f"  Variables: {json.dumps(state.variables, default=str, indent=4)}")
                click.echo(f"  Metadata: {json.dumps(state.metadata, default=str, indent=4)}")
        except Exception as e:
        ...
        if snapshots:
            for snap in snapshots[:10]:  # Show last 10
                ...
                click.echo(f"  [{s.get('snapshot_id', 'unknown')[:8]}...] "
        ...
        if verbose:
            ...
            events = engine.journal.get_events(
                workflow_id, config.org_id, 
                after_seq=-1, limit=10
            )
            for event in events:
...
def time_travel(ctx, workflow_id, savepoint_id, dry_run):
    ...
    try:
        ...
        try:
            ...
            click.echo("Use 'contd inspect <workflow_id>' to list available savepoints", err=True)
            sys.exit(1)
        ...
        if dry_run:
            ...
            click.echo(f"  New workflow ID would be: {workflow_id}-tt-{uuid.uuid4().hex[:8]}")
            return
