...
events_appended_total = Counter(
    "contd_events_appended_total",
    "Total events appended to journal",
    ["event_type"]
)
...
journal_size_bytes = Gauge(
    "contd_journal_size_bytes",
    "Journal size per workflow",
    ["workflow_id"]
)

snapshot_count = Gauge(
    "contd_snapshot_count",
    "Number of snapshots",
    ["workflow_id"]
)
...
class MetricsCollector:
    ...
    def record_restore(
        ...
        restore_duration_milliseconds.labels(
            workflow_name=workflow_name,
            has_snapshot=str(had_snapshot)
        ).observe(duration_ms)
        
        events_replayed_per_restore.labels(
            workflow_name=workflow_name
        ).observe(events_replayed)
        
        workflows_resumed_total.labels(
            workflow_name=workflow_name
        ).inc()
    ...
    def record_snapshot(
        ...
        snapshot_save_duration_milliseconds.labels(
    ...
    def record_journal_append(
        self,
        event_type: str,
        duration_ms: float
    ):
        """Record journal append"""
        journal_append_duration_milliseconds.labels(
    ...
    def record_lease_acquisition(
        ...
        lease_acquisition_duration_milliseconds.labels(
            result=result
        ).observe(duration_ms)
    ...
    def record_cost_savings(
        ...
        avoided_recomputation_steps_total.labels(
            workflow_name=workflow_name
        ).inc(steps_avoided)
    
    def record_data_corruption(
        self,
        data_type: str,
        workflow_id: Optional[str] = None
    ):
        """Record data corruption (CRITICAL)"""
        checksum_validation_failures_total.labels(
            data_type=data_type
        ).inc()
