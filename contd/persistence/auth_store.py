...
from contd.models.auth import UserInDB, Organization, ApiKey, UserRole, OrganizationMember

class AuthStore:
    def __init__(self, db):
        self.db = db

    def create_user(self, email: str, password_hash: str, full_name: str = None) -> UserInDB:
        user_id = uuid4()
        now = datetime.utcnow()
        self.db.execute(
            "INSERT INTO users (user_id, email, password_hash, full_name, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
            str(user_id), email, password_hash, full_name, now, now
        )
        return UserInDB(user_id=user_id, email=email, password_hash=password_hash, full_name=full_name, created_at=now, updated_at=now)
    ...
    def add_org_member(self, org_id: str, user_id: str, role: UserRole):
        self.db.execute(
            ...
            str(org_id), str(user_id), role.value
        )
    ...
    def create_api_key(self, org_id: str, name: str, key_prefix: str, key_hash: str, scopes: List[str], user_id: str = None) -> ApiKey:
        key_id = uuid4()
        now = datetime.utcnow()
        self.db.execute(
            """INSERT INTO api_keys (key_id, key_prefix, key_hash, org_id, user_id, name, scopes, created_at) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            str(key_id), key_prefix, key_hash, str(org_id), str(user_id) if user_id else None, name, scopes, now
        )
        return ApiKey(
            key_id=key_id, key_prefix=key_prefix, org_id=org_id, user_id=user_id, 
