...
from contd.models.auth import UserCreate, User, Token, OrganizationCreate, Organization, ApiKeyCreate, ApiKey
from contd.persistence.auth_store import AuthStore
from contd.api.dependencies import get_auth_store, get_current_user, get_auth_context, AuthContext
from contd.core.auth import get_password_hash, verify_password, create_access_token, generate_api_key
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), store: AuthStore = Depends(get_auth_store)):
...
async def create_api_key_endpoint(
    ...
    if ctx.is_service:
        raise HTTPException(status_code=403, detail="API Keys cannot create new API Keys")

    raw_key, key_hash = generate_api_key()
    
    # Prefix is usually first few chars
    prefix = raw_key.split('_')[0] + "_" + raw_key.split('_')[1] + "_" # e.g. sk_live_
