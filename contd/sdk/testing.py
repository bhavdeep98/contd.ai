...
from contd.sdk.errors import WorkflowInterrupted, ContdError
from contd.core.engine import ExecutionEngine, EngineConfig, MockDB, MockS3
...
class MockExecutionEngine(ExecutionEngine):
    ...
    def set_fail_at(self, step_number: int, exception_type: Type[Exception] = Exception):
    ...
    def check_failure(self, step_number: int):
        ...
        if self._fail_at_step is not None and step_number == self._fail_at_step:
            raise (self._fail_with or Exception)(f"Injected failure at step {step_number}")
...
class ContdTestCase:
    ...
    def assert_interrupted(self, at_step: Optional[int] = None, message: str = ""):
        ...
        if at_step is not None and self.current_execution.interrupted_at_step != at_step:
            raise AssertionError(
    ...
    def assert_failed(self, error_contains: Optional[str] = None, message: str = ""):
        ...
        if error_contains and error_contains not in (self.current_execution.error or ""):
            raise AssertionError(
    ...
    def assert_event_count(self, expected: int, event_type: Optional[str] = None, message: str = ""):
...
class WorkflowTestBuilder:
    ...
    def fail_at(self, step: int, exception: Type[Exception] = Exception) -> 'WorkflowTestBuilder':
    ...
    def assert_interrupted(self, at_step: Optional[int] = None) -> 'WorkflowTestBuilder':
        """Assert workflow was interrupted."""
        self.test_case.assert_interrupted(at_step)
        return self
    
    def assert_failed(self, error_contains: Optional[str] = None) -> 'WorkflowTestBuilder':
